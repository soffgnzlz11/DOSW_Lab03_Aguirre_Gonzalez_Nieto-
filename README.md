...
Activity: Express Hackathon 2026-2 Work mode: Teams of three students
...
Select a diagramming tool of your choice that supports UML and C4 Model standards. Suggestions: Miro, Draw.io or Lucidchart.
Select a mockup and user flow design tool of your choice. Suggestions: Miro or Figma.
...
The TechCup logo is available at the following link.
...
With the goal of gaining a better understanding of the different uses of Maven, you will create a project using Maven archetypes.

Create the feature/proj-structure branch based on the develop branch.
...
Create the basic project structure using the maven-archetype-quickstart archetype (reference).

When running the command to create the project using the archetype, make sure to define:
groupId: edu.eci.dosw.lab
artifactId: DOSW-Laboratorio4
Add the command used to create the project structure to the README.
...
Manually add the docs package (including its subfolders) to obtain the following structure:
DOSW-Lab3/
├── pom.xml
├── .gitignore
├── README.md
...
└── docs/
    ...
    ├── images/
    ├── requirements/
Create a pull request to merge the changes from the feature/proj-structure branch into develop. The pull request must be approved by at least one team member.
...
In the selected tool for C4 diagrams, build the Context Diagram for TechCup.

Create the feature/proj-scope branch based on the develop branch.

Export the context diagram you created as an image and save it in the uml folder.

Create the scope.md file in the requirements folder (path: DOSW-Laboratorio4/docs/requirements). Note: Use the document in the link as a template.

Complete each section of the file. Important: You can fill it in Spanish:

System.
Problem to be solved.
Context Diagram: (Include the image and link to the diagram created in step 1).
System Scope.
Create a pull request to merge the changes from the feature/proj-scope branch into develop. The pull request must be approved by at least one team member.
...
Create the feature/proj-requirements branch based on the develop branch.

Create the requirements.md file in the requirements folder (path: DOSW-Lab3/docs/requirements). Note: Use the document in the link as a template.

List the main functional and non-functional requirements of the TechChup system (at least 5 of each type of requirement). It is not necessary to provide detailed descriptions; mention each requirement in a single sentence.

From the list, select 3 functional requirements of the TechCup system and create a use case diagram for each one. The diagrams must follow the UML standard.

Export the use case diagrams as images and save them in the uml folder.

Provide a detailed description of each of the 3 functional requirements selected in step 4. Use the template provided in the requirements.md file.
...
Create a pull request to merge the changes from the feature/proj-requirements branch into develop. The pull request must be approved by at least one team member.
...
Create the feature/proj-design branch based on the develop branch.

Select one of the functional requirements described in Part 3. Consider the following suggestions:

Select the requirement for which you have the most detailed information and the least uncertainty.
Select the requirement that includes enough visual elements to allow you to apply different design resources. If none of the requirements meet this criterion, select one and propose the visual elements yourself.
Select a requirement that has at least 3 steps in the main flow. If none of the requirements meet this criterion, select one and define the 3 steps yourself.
Design the mockups for the screens involved in the selected functional requirement and its navigation flow. Design a minimum of 3 and a maximum of 5 screens. Make sure to use the TechCup logo.

Export the mockups as images and save them in the images folder.

Update the requirements.md file in your repository by including a link to the mockup associated with the selected requirement.

Create a pull request to merge the changes from the feature/proj-design branch into develop. The pull request must be approved by at least one team member.
